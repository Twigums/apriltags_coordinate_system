...
def find_corners(image,
    ...
    for contour in contours:
        ...
        # first two outputs are boundingRect's x and y respectively
    ...
    # can probably increase this to be half of the image?
...
def get_detection(sift: cv2.SIFT,
    ...
    for _ in range(max_iterations): 
        ...
        if len(good_matches) > min_match_count:
            ...
            # second output is mask (for inlier checking)
            H, _= cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, ransac_threshold)

            # init return variables for error checking
            transformed_box = None
            iou = 0

            # apply transformation to rectangle if homography matrix exists
            if H is not None:
